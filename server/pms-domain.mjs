...
export function getRoomTypeForNumber(roomNumber) {
  return String(roomNumber).startsWith('2') ? 'TWIN' : 'DOUBLE'
}

export function isSellableRoomNumber(roomNumber) {
  const value = String(roomNumber)
  return [...SANDBOX_RULES.twinRooms, ...SANDBOX_RULES.doubleRooms].includes(value) &&
    !SANDBOX_RULES.nonSellableRooms.includes(value)
}
...
export function calculateStayPricing(input) {
  ...
  const extraAdults = Math.max(0, adults - SANDBOX_RULES.standardOccupancy)
  ...
  const extraGuestFee = extraAdults * SANDBOX_RULES.extraGuestFeePerNight * nights
  const childFee = chargedChildren * SANDBOX_RULES.childSharingFeePerNight * nights
