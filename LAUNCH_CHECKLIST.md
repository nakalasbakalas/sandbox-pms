...
Local candidate note (2026-08-02): controlled booking-email autonomy is implemented on `codex/bookinginbox` with the global switch disabled by default. The full launch gate and guarded disposable-database suite pass. The live Gmail identity/provider-domain families, canonical Render service/database, and redacted backend OAuth readiness were inspected. The migration is not yet deployed, live room mappings and staff acceptance remain open, and production autonomy is not enabled. See `docs/launch/evidence/2026-08-02-booking-email-controlled-autonomy.md`.
