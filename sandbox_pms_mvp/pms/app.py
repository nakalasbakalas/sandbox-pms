...
from .routes.provider import register_provider_routes
...
""".strip()
    ...
    def staff_cashier_payment(reservation_id):
        ...
        try:
            ...
            if request.form.get("email_receipt") == "on":
                try:
                    reservation = db.session.get(Reservation, reservation_id)
                    payment_request_id = None
                    if payment_line.metadata_json:
                        raw_payment_request_id = payment_line.metadata_json.get("payment_request_id")
                        if raw_payment_request_id:
                            payment_request_id = UUID(str(raw_payment_request_id))
                    payment_request = db.session.get(PaymentRequest, payment_request_id) if payment_request_id else None
                    if not reservation or not payment_request:
                        raise ValueError("Receipt email could not be prepared for this payment.")
                    if not reservation.primary_guest or not reservation.primary_guest.email:
                        raise ValueError("Guest email is not available for receipt delivery.")
                    receipt = issue_cashier_document(
                        reservation_id,
                        DocumentIssuePayload(
                            document_type="receipt",
                            note="Receipt emailed after manual payment",
                        ),
                        actor_user_id=user.id,
                    )
                    delivery_ids = queue_cashier_receipt_email(
                        reservation,
                        payment_request,
                        receipt,
                        actor_user_id=user.id,
                    ...
                    flash("Payment recorded and receipt email queued.", "success")
                except Exception as email_exc:  # noqa: BLE001
                    flash("Payment recorded on folio.", "success")
                    flash(f"Receipt email was not queued: {public_error_message(email_exc)}", "warning")
            else:
                flash("Payment recorded on folio.", "success")
        except Exception as exc:  # noqa: BLE001
            flash(public_error_message(exc), "error")
