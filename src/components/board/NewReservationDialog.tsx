...
export function NewReservationDialog({ open, onClose, onSubmit, prefilledData }: NewReservationDialogProps) {
  ...
  const handleRoomTypeChange = (value: 'TWIN' | 'DOUBLE') => {
  ...
  useEffect(() => {
    ...
    setFormData((current) => {
      ...
      if (!selected || current.ratePerNight > 0) return current
      return {
        ...current,
  ...
  const handleSubmit = async () => {
    ...
    setFormData({
      ...
      adults: 1,
      children: 0,
      ratePerNight: roomTypeOptions[0]?.baseRate || 0,
      source: 'DIRECT',
      specialRequests: '',
    })
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <Select value={formData.roomType} onValueChange={(v) => handleRoomTypeChange(v as 'TWIN' | 'DOUBLE')}>
                ...
                {roomTypeOptions.length === 0 && (
                  <p className="mt-1 text-xs text-destructive">Room types are not configured.</p>
                )}
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="ratePerNight" className="text-xs">Rate per Night ({currency})</Label>
                <Input
                  id="ratePerNight"
                  type="number"
                  min="0"
            ...
            <Card className="p-2.5 bg-muted">
              ...
              <div className="space-y-0.5 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{currency} {formData.ratePerNight.toLocaleString()}</span>
                </div>
