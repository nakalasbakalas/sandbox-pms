...
const defaultSettings: GrowthSuiteSettings = {
  ...
  promoCode: 'DIRECT10',
  bookingUrl: 'book.sandboxhotel.local',
}
...
const roomInventory = [
  { type: 'Standard Twin', available: 6, baseRate: 1850, directShare: 31, extras: ['Breakfast', 'Airport pickup'] },
  { type: 'Deluxe Double', available: 4, baseRate: 2350, directShare: 27, extras: ['Late checkout', 'Welcome set'] },
  { type: 'Pool View Suite', available: 2, baseRate: 4100, directShare: 18, extras: ['Private transfer', 'Dinner package'] },
]

const campaigns: Campaign[] = [
  { channel: 'Google Hotel Ads', status: 'paused', spend: 4200, bookings: 11, roas: 4.6 },
  { channel: 'Tripadvisor', status: 'draft', spend: 1800, bookings: 3, roas: 2.9 },
  { channel: 'Trivago', status: 'draft', spend: 1300, bookings: 2, roas: 2.3 },
]
...
export function GrowthSuiteView() {
  ...
  const [selectedRoomType, setSelectedRoomType] = useState(roomInventory[0].type)
  ...
  }, [currentSettings.directMarkup])

  const directConversion = currentSettings.bookingEngineEnabled ? 14.2 : 0
  const siteReadiness = [
      ...
      <ScrollArea className="h-[calc(100vh-6.25rem)]">
        <div className="space-y-6 p-6">
          <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <MetricTile label="Direct share" value="28%" detail="+4.1% vs last month" icon={TrendUp} />
            <MetricTile label="Conversion" value={`${directConversion.toFixed(1)}%`} detail="Website to booking" icon={Storefront} />
            <MetricTile label="Booking value" value="THB 312k" detail="Direct revenue this month" icon={CreditCard} />
              ...
              <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
                ...
                <Card className="rounded-lg">
                  <CardHeader>
                    ...
                    <CardDescription>{selectedInventory.type}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      {selectedInventory.extras.map(extra => (
                        <div key={extra} className="flex items-center justify-between rounded-md border px-3 py-2 text-sm">
                          <span>{extra}</span>
                          <Badge variant="secondary" className="rounded-md text-[10px]">Addon</Badge>
                        </div>
                      ))}
                    </div>

                    <Button className="w-full" onClick={() => toast.success('Test direct booking created in the sandbox')}>
                      Create Test Booking
                    </Button>
                  </CardContent>
                </Card>
