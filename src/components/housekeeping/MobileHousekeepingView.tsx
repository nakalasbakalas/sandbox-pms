...
export function MobileHousekeepingView() {
  ...
  const floors = useMemo(() => {
    const grouped = new Map<string, HousekeepingRoom[]>()
    for (const room of rooms) {
      const floor = Number.isFinite(room.floor) ? String(room.floor) : 'Unassigned'
        ...
        <div className="space-y-3">
          {floors.map((floorGroup) => (
            <FloorSection
              ...
              title={floorGroup.floor === 'Unassigned' ? 'Unassigned Floor' : `Floor ${floorGroup.floor}`}
              rooms={floorGroup.rooms}
              expanded={expandedFloors[floorGroup.floor] ?? true}
              onToggle={() => toggleFloor(floorGroup.floor)}
