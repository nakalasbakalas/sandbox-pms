        ...
        <ScrollArea className="h-[400px]">
          <div className="space-y-2">
            {inventory.map((snap, index) => {
              const percentage = (snap.availableUnits / snap.totalUnits) * 100
...
export function InventoryOverview() {
  ...
  const roomTypes = [
    { id: 'deluxe', name: 'Deluxe Room', total: 10 },
    { id: 'superior', name: 'Superior Room', total: 8 },
    { id: 'suite', name: 'Suite', total: 5 }
  ]
  ...
  const overviewStats = roomTypes.map(roomType => {
    ...
    const avgOccupancy = ((totalReserved / (roomType.total * 7)) * 100).toFixed(1)
