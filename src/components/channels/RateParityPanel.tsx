...
export function RateParityPanel({ connectedChannels }: RateParityPanelProps) {
  ...
  const handleCheckNow = async () => {
    ...
    setChecking(false)
    toast.info('Rate parity check completed', {
      description: 'Missing live channel rates are reported without generated data.',
    })
  }
  ...
  const hasParityData = parityChecks.length > 0
    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {hasParityData ? getScoreStatus(overallScore) : 'No checks run'}
            </p>
          </CardContent>
        </Card>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={checking || enabledChannels.length === 0 || roomTypes.length === 0}
          >
      ...
      <div className="grid grid-cols-3 gap-4">
        {channelHealthData.map(health => {
          const channelHasData = parityChecks.some((check) => check.channelsChecked.includes(health.channelId))
          return (
            ...
            <CardContent className="space-y-3">
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm text-muted-foreground">Parity Score</span>
                  <span className={cn("text-sm font-bold", channelHasData ? getScoreColor(health.parityScore) : 'text-muted-foreground')}>
                    {channelHasData ? `${health.parityScore}%` : 'No data'}
                  </span>
                </div>
                <Progress value={channelHasData ? health.parityScore : 0} className="h-2" />
              </div>
