  ...
  const handleManualSync = async () => {
    ...
    try {
      for (const roomType of roomTypes) {
        await manualSyncAllChannels(roomType.id, 90)
      }
    } finally {
      setSyncing(false)
    }
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <Button
                  ...
                  disabled={syncing || connectedChannels.filter(ch => ch.connected).length === 0 || roomTypes.length === 0}
                >
            ...
            <div>
              ...
              <div className="space-y-2">
                {channelHealth.map((health) => {
                  ...
                  const hasLogs = syncLogs.some((log) => log.channelId === health.channelId)
                    ...
                    <div key={health.channelId} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          ...
                          <p className="text-sm font-bold">{hasLogs ? `${health.successRate.toFixed(1)}%` : 'No data'}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-muted-foreground">Avg Time</p>
                          <p className="text-sm font-bold">{hasLogs ? `${(health.avgDuration / 1000).toFixed(1)}s` : 'No data'}</p>
                        </div>
                        <Badge variant="outline" className={cn("text-xs", hasLogs ? getHealthColor(health.health) : 'text-gray-600 bg-gray-50 border-gray-200')}>
                          {hasLogs ? health.health : 'No data'}
                        </Badge>
                      </div>
                    </div>
                  )
