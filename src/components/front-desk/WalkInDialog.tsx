...
export function WalkInDialog({ open, rooms, role, onOpenChange, onConfirm }: WalkInDialogProps) {
  ...
  const roomTypeOptions = useMemo(() => {
    if (configuredRoomTypes.length === 0) {
      return [
        { code: 'TWIN' as const, name: 'Standard Twin', baseRate: 2000, baseOccupancy: 2, maxOccupancy: 2, extraGuestFee: 300, childFee: 300 },
        { code: 'DOUBLE' as const, name: 'Superior Double', baseRate: 2000, baseOccupancy: 2, maxOccupancy: 4, extraGuestFee: 300, childFee: 300 },
      ]
    }

    return configuredRoomTypes.map((type, index) => ({
      code: type.code || (type.name.toLowerCase().includes('double') || type.id.toLowerCase().includes('double') || index === 1 ? 'DOUBLE' : 'TWIN') as 'TWIN' | 'DOUBLE',
      name: type.name,
      baseRate: Number(type.baseRate || 0),
      baseOccupancy: Number(type.baseOccupancy || 2),
      maxOccupancy: Number(type.maxOccupancy || 2),
      extraGuestFee: Number(type.extraGuestFee || 300),
      childFee: Number(type.childFee || 300),
    }))
  }, [configuredRoomTypes])
  const selectedRoomType = roomTypeOptions.find((type) => type.code === roomType) || roomTypeOptions[0]
  const currency = propertyData?.currency?.trim() || 'THB'

  useEffect(() => {
    if (ratePerNight > 0 || !selectedRoomType?.baseRate) return
    setRatePerNight(selectedRoomType.baseRate)
  }, [ratePerNight, selectedRoomType])
  ...
  const resetAndClose = (nextOpen: boolean) => {
    if (!nextOpen) {
      ...
      setRoomType('TWIN')
      setNights(1)
      setRatePerNight(roomTypeOptions[0]?.baseRate || 0)
      setSelectedRoomId('')
      setPaymentAmount(0)
          ...
          <section className="grid gap-3 md:grid-cols-4">
            <label className="space-y-1">
              ...
              <Select value={roomType} onValueChange={(value) => {
                ...
                const nextRoomType = roomTypeOptions.find((option) => option.code === type)
                setRoomType(type)
                setRatePerNight(nextRoomType?.baseRate || 0)
                setSelectedRoomId('')
              }}>
            ...
            <div className="rounded-md border bg-muted/30 px-3 py-2">
              ...
              <div className="font-semibold">{currency} {pricing.total.toLocaleString('en-US')}</div>
            </div>
          </section>
