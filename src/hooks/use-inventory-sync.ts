...
export function useInventorySync() {
  ...
  const [autoSyncEnabled, setAutoSyncEnabled] = useKV<boolean>('auto-sync-enabled', false)
  const [rooms] = useKV<BoardRoomCard[]>('pms-rooms', [])
  const [roomTypes] = useKV<RoomTypeConfig[]>('room-types-config', [])
  ...
  const calculateInventoryForDate = useCallback((roomTypeId: string, date: string): InventorySnapshot => {
    ...
    const typeLabel = roomType?.name.toLowerCase() || roomTypeId.toLowerCase()
    const matchingRooms = (rooms || []).filter((room) => {
      if (room.roomTypeId) return room.roomTypeId === roomTypeId
      const code = (room.roomType || room.type || '').toLowerCase()
      return code === roomTypeId.toLowerCase() || typeLabel.includes(code)
    })
    const totalUnits = matchingRooms.length
    const blockedUnits = matchingRooms.filter((room) => room.operationalStatus !== 'AVAILABLE').length
    const reservedUnits = matchingRooms.filter((room) =>
      Boolean(room.guestName || room.reservationId || room.currentReservationId),
    ).length
  ...
  const simulateChannelSync = async (_channelId: string, _updates: any[]): Promise<void> => {
    throw new Error('No live channel connector configured')
  }
  ...
  const manualSyncAllChannels = useCallback(async (roomTypeId: string, days: number = 90) => {
    ...
    try {
      ...
      toast.info('Inventory sync request recorded', {
