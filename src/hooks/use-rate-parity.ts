...
export function useRateParity() {
  ...
  const fetchChannelRate = useCallback(async (
    channelId: string,
    roomTypeId: string,
    date: string
  ): Promise<number> => {
    const existingRate = rateSnapshots.find(
      (snapshot) => snapshot.roomTypeId === roomTypeId && snapshot.date === date,
    )?.channelRates[channelId]

    if (Number.isFinite(existingRate)) {
      return Number(existingRate)
    }

    throw new Error('No live channel rate available')
  }, [rateSnapshots])
  ...
  const checkRateParity = useCallback(async (
    ...
    const roomTypeRate = roomTypes.find((roomType) => roomType.id === roomTypeId)
    const pmsRate = Number(roomTypeRate?.baseRate || 0)

    if (!Number.isFinite(pmsRate) || pmsRate <= 0) {
      check.duration = Date.now() - startTime
      check.status = 'FAILED'
      check.errors = [`${roomTypeId}: No configured PMS base rate`]
    ...
    check.duration = Date.now() - startTime
    check.status = errors.length > 0 ? (successfulRates > 0 ? 'PARTIAL' : 'FAILED') : 'SUCCESS'
    if (errors.length > 0) {
      check.errors = errors
    }
    ...
    return check
  }, [fetchChannelRate, roomTypes, setRateSnapshots, setParityChecks, setViolations, settings.alertThreshold])
  ...
  const getOverallParityScore = useCallback(() => {
    ...
    if (totalCheckedRates === 0) return 0
