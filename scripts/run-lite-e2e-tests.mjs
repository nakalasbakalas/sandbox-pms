...
async function runCoreOperationsProof(adminContext, adminPage, frontDeskPage, frontDeskErrors, baseUrl, reservation, runId) {
  ...
  await visible(bookingRow, 'booking row for folio workflow')
  await bookingRow.getByRole('button', { name: 'Folio', exact: true }).click()
  const folioDialog = adminPage.getByRole('dialog', { name: `Folio · ${reservation.confirmationCode}`, exact: true })
  await visible(folioDialog, 'folio detail dialog')
