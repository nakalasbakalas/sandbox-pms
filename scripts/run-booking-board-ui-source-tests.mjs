...
const reservationsDialogSource = (await readFile('src/components/reservations/ReservationsView.tsx', 'utf8')).replaceAll('\r\n', '\n')
...
const frontDeskSource = (await readFile('src/components/front-desk/FrontDeskView.tsx', 'utf8')).replaceAll('\r\n', '\n')
const guestsSource = (await readFile('src/components/views/GuestsView.tsx', 'utf8')).replaceAll('\r\n', '\n')
const legacyBoardSource = (await readFile('src/components/board/Board.tsx', 'utf8')).replaceAll('\r\n', '\n')
...
assert.match(source, /operation: 'reservation-create'/, 'the server Booking Board retains one durable key for an unchanged reservation create')
assert.match(source, /headers: \{ 'x-idempotency-key': idempotencyKey \}/, 'the server Booking Board sends its retained create key')
assert.match(reservationsSource, /operation: 'reservation-create'/, 'Reservations retains one durable key for an unchanged reservation create')
assert.match(reservationsSource, /headers: \{ 'x-idempotency-key': idempotencyKey \}/, 'Reservations sends its retained create key')
assert.match(reservationsDialogSource, /onSubmit: \(reservation: ReservationData\) => Promise<void> \| void/, 'the Reservations create dialog accepts an asynchronous authoritative submission')
assert.match(reservationsDialogSource, /await onSubmit\(reservation\)[\s\S]{0,520}setFormData/, 'the Reservations create dialog clears operator input only after the server confirms success')
assert.match(reservationsDialogSource, /catch \(error\)[\s\S]{0,240}Reservation could not be created/, 'the Reservations create dialog keeps input and surfaces an authoritative failure')
assert.match(frontDeskSource, /operation: 'reservation-create'/, 'Front Desk retains one durable key for an unchanged reservation create')
assert.match(frontDeskSource, /headers: \{ 'x-idempotency-key': idempotencyKey \}/, 'Front Desk sends its retained create key')
assert.match(legacyBoardSource, /operation: 'reservation-create'/, 'the compatibility Board retains one durable key for an unchanged reservation create')
assert.match(legacyBoardSource, /headers: \{ 'x-idempotency-key': idempotencyKey \}/, 'the compatibility Board sends its retained create key')
assert.match(guestsSource, /operation: 'guest-create'/, 'Guests retains one durable key for an unchanged guest create')
assert.match(guestsSource, /headers: \{ 'x-idempotency-key': idempotencyKey \}/, 'Guests sends its retained create key')
assert.match(attemptSource, /'reservation-create'/, 'durable attempt keys allow reservation-create retries')
assert.match(attemptSource, /'guest-create'/, 'durable attempt keys allow guest-create retries')
