...
- Server-mode cashier, front-desk, reservation, and booking-board submissions use `DurableAttemptKeyManager`. It stores only a hashed operation/entity slot, intent fingerprint, and opaque key in same-tab `sessionStorage`, reuses the key for an unchanged uncertain attempt across a full reload, rotates when material input changes, and removes it only after confirmed success. It never stores the request material or raw entity identifier and never uses `localStorage`; the backend remains authoritative for replay validation.
