...
**Status: ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ COMPLETE (2026-03-18)**
...
| **F-01** SSE blocks workers | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Fixed | SSE endpoint removed; replaced with 10s JS polling |

| **F-02** Render storage config | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Fixed | `Config` now loads local/S3 storage env vars and `render.yaml` enables a persistent Render disk at `/var/data/uploads/documents` |

| **F-03** Background tasks unwired | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Fixed | 9 Render cron jobs wired, including notifications, automation, iCal sync, waitlist, audit cleanup, and no-show auto-cancel |

| **F-07** Root cleanup | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Fixed | 28 artifacts deleted, 4 docs moved to `docs/`, 4 root tests removed |

| **F-09** FEATURE_BOARD_V2 unclear | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Fixed | Dead config and startup gate removed; board action surface stays always on via compatibility helper |

| **F-10** OCR returns None silently | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Fixed | Now returns status dict: `{"status": "unavailable", ...}` |

| **QW-1 to QW-11** | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Fixed | All quick wins completed (aria-labels, guards, config) |
...
**Status: ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ COMPLETE (2026-03-19)**
...
| Departure turnover auto-wiring | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Verified | Already wired in `complete_checkout` |

| OCR unavailability status | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Fixed | Returns `{"status": "unavailable", ...}` (done in Phase 0) |

| Root cleanup | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Fixed | Done in Phase 0 |

| `FEATURE_BOARD_V2` flag | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Removed | Legacy env/config removed; board action surface is always on |

| DB liveness `/health` | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Verified | Already implemented with `SELECT 1` |
...
| Stripe webhook idempotency | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Verified | 3-layer protection: (1) `provider_event_id` unique index, (2) `posting_key` unique on FolioCharge, (3) `SELECT FOR UPDATE` on PaymentRequest. Concurrent threading tests exist. |
...
| Report CSV downloads | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | `build_csv_rows()` + `/staff/daily-reports/<type>/csv` route + Download CSV button. All 7 report types supported. |

| Modification request approve/decline with auto-pricing | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | `approve_modification_request()` (applies `change_stay_dates` + reprices), `decline_modification_request()`, `quote_modification_request()` (JSON API). UI in reservation detail template with confirm dialog. |

| Folio balance badge on front desk board | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | Batch `SUM(FolioCharge.total_amount)` query per visible reservation (single GROUP BY, zero N+1). Badge shown as `currency + amount` on each block; also in popover detail. |

| Ready-for-check-in badge on front desk board | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | `readyForCheckIn` flag computed per block: confirmed + arrival ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¤ today + allocated + room ready (clean/inspected, not blocked). Green "Ready" badge on block. |

| Pending modification indicator on board + workspace | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | Batch `COUNT(ModificationRequest)` per visible reservation (single GROUP BY). "Mod" badge on board blocks + "Mod" link badge in workspace Flags column. Front desk workspace includes `pending_modification_requests` count. |

| Pricing module tests | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | 16 direct tests in `test_pricing.py`: `apply_adjustment` (5 tests), `money` helper, `nightly_room_rate` (5 tests: fallback, fixed override, day-of-week filter, min_nights, long-stay discount), `quote_reservation` (5 tests: basic, extra guest fee, child fee, zero VAT, VAT breakdown consistency). |
...
**Fix:** Either switch to `gevent`/`eventlet` worker class for async I/O, move SSE behind a dedicated worker, replace the polling SSE with a cheaper full-page refresh or HTMX polling pattern, or replace with periodic JSON polling (every 5ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“10 seconds) which is far simpler and has no blocking risk.
...
**Evidence:** 8 Flask CLI commands in `app.py:627ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“720` (`process-notifications`, `send-pre-arrival-reminders`, `send-failed-payment-reminders`, `fire-pre-checkin-reminders`, `sync-ical-sources`, `process-automation-events`, etc.). `render.yaml` has no `jobs:` or `cron:` section.  
...
**Evidence:** `public_booking_service.py:355ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“388` `_check_public_rate_limit()` runs two `COUNT` SQL queries per booking/cancel/modification attempt. `auth_service.py:444ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“455` `ip_is_rate_limited()` counts `AuthAttempt` rows.  
...
- [ ] Add housekeeping supervisor inspection workflow (inspect task ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ pass/fail ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ re-queue if failed)

- [ ] Add room maintenance request from guest-facing channel (guest reports issue ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ auto-creates maintenance task)
...
- [x] Add integration test for `LocalStorageBackend` round-trip (save ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ read ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ serve) *(covered on 2026-03-20 in `test_phase17_pre_checkin.py`: local backend upload writes to configured `UPLOAD_DIR`, `read_document_bytes()` returns the original bytes, and `get_document_serve_url()` returns `None` for direct app serving.)*
...
- [x] ~~Add departure turnover task auto-wiring to checkout flow~~ ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Verified: already wired in `complete_checkout`
...
- [x] Fix Stripe webhook idempotency ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Verified: 3-layer protection (provider_event_id unique + posting_key unique + SELECT FOR UPDATE). Concurrent threading tests exist.
...
- [x] Add explicit "ready for check-in" status badge *(computed flag: confirmed + arrival ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¤ today + allocated + room ready)*
...
| Shared `helpers.py` module | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | ~60 functions extracted (auth, CSRF, parsing, utility, branding, i18n, reports) |

| Auto-cancel same-day no-shows | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | `auto_cancel_no_shows()` service + CLI + render.yaml cron (21:00 UTC) |

| Guest visit history view | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | `GET /staff/guests/<id>` route + `staff_guest_detail.html` template |
...
| Template URL endpoint updates | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | 18 templates updated to use blueprint-prefixed `url_for()` calls after route extractions |
...
| Front desk board density persistence | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | `UserPreference` persistence confirmed; test_phase15_front_desk_board.py (63 passed, 2026-03-19) |
...
| Early check-in / late check-out fee automation | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | Implemented in front-desk check-in/out flows; test_phase6_front_desk_workspace.py green (27 passed, 1 skipped, 2026-03-20) |
...
| Departure turnover task auto-wiring | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | Verified wired to `complete_checkout`; confirmed in Phase 1 review |

| Staff skip-navigation link | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done | Confirmed in `templates/base.html`; guardrail tests pass |
...
1. Fix UPLOAD_DIR / storage ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ documents survive deploys

2. Fix MAX_CONTENT_LENGTH ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ documents > 1 MB actually upload

3. Wire cron jobs ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ automated emails fire without manual CLI

4. Fix SSE worker-blocking ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ board works under concurrent load

5. Auto-wire checkout ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ housekeeping turnover tasks appear reliably

6. Fix Stripe idempotency ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ no double-payment postings

7. Add modification request re-pricing ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ finance workflow complete

8. Add report CSV exports ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ management self-service

```



Steps 1ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“4 are Phase 0 (configuration/small changes). Steps 5ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“8 are Phase 1ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“2 code changes. Everything beyond this is quality-of-life and feature expansion.
