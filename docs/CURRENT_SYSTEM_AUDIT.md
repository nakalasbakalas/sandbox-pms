...
- The Lite migration chain now includes manual queue, Gmail push, satang expansion/backfill, mapping-target uniqueness, immutable task targets/read-only legacy email evidence, provider-total provenance, and the `RateCalendar` satang/provider-storage follow-up. The guarded `npm.cmd run test:money-backfill:db` command seeds Float-only legacy rows in an isolated PostgreSQL schema, applies the money migrations, checks exact backfill/quarantine, and runs the aggregate reconciliation before cleanup. This is engineering proof only and must pass for the exact commit; none of these migrations is proven on Render Lite staging or production.
