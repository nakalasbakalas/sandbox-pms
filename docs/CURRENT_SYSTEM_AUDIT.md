...
- Booking email intake: backend routes exist for status, sync, events, approve/reject/reprocess, and sources; Gmail mailbox sync supports backend-owned OAuth access-token or refresh-token credentials. Historical capture can be checked with `npm.cmd run booking-email:backfill -- --all-past --limit <n>`; the helper is dry-run by default and confirmed imports create review-only Booking Email Events for `/booking-inbox`.
