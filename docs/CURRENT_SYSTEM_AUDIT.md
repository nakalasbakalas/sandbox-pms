...
- In server mode, Settings, Rooms, Booking Board/Front Desk lifecycle handoffs, Cashier folio selection, housekeeping, and Night Audit use backend authority. Rooms reads the authenticated Board snapshot and fails closed rather than rendering browser-KV room/property/type data; disposable-browser coverage is engineering evidence and does not replace staff workflow acceptance on the exact release candidate.
