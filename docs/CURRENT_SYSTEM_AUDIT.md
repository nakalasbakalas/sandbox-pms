...
- Booking Board operations: the server-authoritative timeline selects unassigned or assigned stays and submits room assignment/move, stay-date resize, reason-required cancellation/no-show, reservation-scoped guest/VIP updates, and exact-satang folio extras through authenticated APIs. Every visible Board mutation uses an opaque per-intent idempotency key, never reports optimistic success, and refetches authoritative state after known success or rejection. The Board response is a permission-filtered DTO: housekeeping can see operational guest identity/VIP state but not contact/profile PII, channel references, notes, or folio values. Room blocks remain display-only; block authoring/clearing and destructive deletion policy remain separate operational work.
