...
- Deterministic Booking.com, Agoda, and Trip.com extraction now covers the live provider layout families inspected through the booking mailbox without storing guest data in fixtures or evidence. Agoda coverage handles bilingual multiline field tables and `D-MMM-YYYY` dates. Trip.com coverage handles accepted-booking subjects, surname/given-name slash form, text-month stay ranges, and rate-plan suffixes appended to the OTA room category. Booking.com coverage handles multiline reservation fields and text-month dates.
