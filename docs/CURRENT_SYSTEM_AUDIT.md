...
- Business, Board, lifecycle, permission, and route smoke tests: `scripts/run-business-tests.mjs`, `scripts/run-front-desk-board-tests.mjs`, `scripts/run-reservation-lifecycle-safety-tests.mjs`, `scripts/run-booking-board-ui-source-tests.mjs`, `scripts/run-front-desk-permission-source-tests.mjs`, `scripts/run-e2e-tests.mjs`, and `scripts/run-server-mode-browser-tests.mjs`.
...
- In server mode, Settings, Rooms, Reservations, Booking Board/Front Desk lifecycle handoffs, Cashier folio selection, housekeeping, and Night Audit use backend authority. Rooms reads the authenticated Board snapshot and Reservations requires authenticated reservation-list plus Board room/readiness snapshots; both fail closed rather than rendering browser-KV operational data. The Reservations unavailable state removes create affordance until Retry restores both snapshots. Disposable-browser coverage is engineering evidence and does not replace staff workflow acceptance on the exact release candidate.
...
The integration CI definition now includes an empty PostgreSQL migration lifecycle, a separately migrated and seeded disposable PostgreSQL database, `scripts/run-release-foundation-db-tests.mjs`, and `scripts/run-server-mode-browser-tests.mjs`. The guarded DB suite covers effective membership roles, missing membership, forged cross-property identifiers and source links, payment/charge and check-in/out property-scoped idempotency, unchanged lifecycle replay without duplicate evidence, stale lifecycle rollback, concurrent lifecycle serialization, concurrent overpayment and charge retry behavior, exact-money reconciliation, audit ownership, and simultaneous last-room holds. The browser suite covers effective-role UI denial, Board-to-Front-Desk/Cashier handoffs, authenticated server-mode reload persistence, controlled API failures, recovery by authoritative refetch, and SSE filtering. These are engineering contracts; they are not claims that GitHub's two required jobs passed the corrected commit, that migrations succeeded on a restored staging copy, or that staff/provider/owner proof exists.
