...
- Render Gmail OAuth env-var status checks must use key-presence output only, such as `npm.cmd run render:gmail-oauth:status -- --use-render-cli-token`, and must omit all values. Env-var setup must be dry-run first with `npm.cmd run render:gmail-oauth`; when applied, the helper may update only the known booking-email Gmail keys and must omit all env-var values and Render auth tokens from output.
