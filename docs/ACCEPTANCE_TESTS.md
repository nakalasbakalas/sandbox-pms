...
- A cancellation fixture containing an oversized THB-like provider marker still parses as `CANCELLATION` with no amount and does not raise a money conversion error.
...
- Workspace booking JSON accepts the observed nested and top-level schemas, a BOM, and trailing non-JSON grade prose while extracting only the first bounded JSON object. Unknown fields, missing/invalid references, oversized documents, non-booking security analyses, and conflicting type/date/room values fail closed.
- Workspace self-grades and justifications never affect confidence. Aligned JSON adds at most 0.02, duplicate provider emails remain a separate corroboration count, and persisted/audited evidence contains no raw JSON, guest value, OAuth token, or grade prose.
- Drive polling requires explicit folder id, enabled flag, and Drive-readonly OAuth scope; it lists at most 100 files and exports at most 20 reference-matched candidates per sync. A missing required Workspace record blocks autonomous writes without blocking review-only Gmail capture.
- Verified LittleHotelier reservation mail is included in the approved Gmail query and parses reference, guest, dates, room label, and THB total. Until its provider mapping contract exists, it remains review-only and cannot auto-assign a room.
...
- A failed iCal configuration keeps its dialog open, shows the sanitized API error inline, and permits an unchanged retry with the same idempotency key; no credential, bearer URL, header, or raw database error is rendered.
...
- collapsed-layout fixtures prove exact Agoda date-token bounds and Trip.com guest-name termination at `Room Type`, preventing unbounded body capture;
