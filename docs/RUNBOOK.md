...
Booking email intake uses `BOOKING_EMAIL_PRIMARY_MAILBOX=booking@sandboxhotel.com`. Do not store a Gmail mailbox password in app config. Server sync requires either `BOOKING_EMAIL_GMAIL_ACCESS_TOKEN` or backend OAuth refresh-token credentials: `BOOKING_EMAIL_GMAIL_CLIENT_ID`, `BOOKING_EMAIL_GMAIL_CLIENT_SECRET`, and `BOOKING_EMAIL_GMAIL_REFRESH_TOKEN`. `BOOKING_EMAIL_GMAIL_SCOPES` defaults to `https://www.googleapis.com/auth/gmail.readonly`; do not broaden it unless a backend feature genuinely needs more access. `/api/booking-email/status` passively reports non-secret credential readiness, missing key names, the target mailbox, and last sync state; it must not call Gmail. Use mailbox sync or the explicit OAuth diagnostic when live connectivity proof is required. For Render, prefer the durable refresh-token tuple. Use `npm.cmd run render:gmail-oauth:status -- --use-render-cli-token` to check current Render key presence without printing values, then use `npm.cmd run render:gmail-oauth` as a dry-run before applying any env-var changes. Operational/security/provider-admin emails from OTA senders are not booking events; keep those rows `UNKNOWN` / review-only unless staff confirm they are actionable reservation communications. When no explicit `--query` is supplied, historical backfill defaults to the approved provider-query boundary instead of the incomplete direct `to:booking@sandboxhotel.com` filter.

For Google Workspace booking JSON corroboration, first confirm the target folder contains only the intended generated booking-analysis files. Re-authorize the booking OAuth client with `npm.cmd run gmail-oauth:render -- --include-drive-readonly-scope --listen --apply-render --use-render-cli-token`; do not reuse an older refresh token as proof that Drive scope was granted. In the Render secret/config surface set `BOOKING_EMAIL_GMAIL_SCOPES` to both Gmail readonly and Drive readonly, set `BOOKING_EMAIL_WORKSPACE_JSON_FOLDER_ID`, then set `BOOKING_EMAIL_WORKSPACE_JSON_ENABLED=true`. Keep `BOOKING_EMAIL_REQUIRE_WORKSPACE_JSON=true`. Verify the non-secret `workspaceJson` status in Booking Inbox before considering autonomy. A missing/misaligned JSON record stays in review; never paste Drive OAuth tokens or JSON guest data into logs.
